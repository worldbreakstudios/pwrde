...
| `?tool=pr` / `local_diff` / `launch` | open that tool panel on the Sessions page |
| `?set=<key>=<value>` (repeatable) | any settings key before boot, e.g. `?page=notes&set=features.notes=true` |
...
so the pages take their normal path from "loading" to "ready". The PR diff
and the local-diff tool are answered from `src/fixtures/pr.diff`, run through
the same `build_diff_render` the worker uses.
