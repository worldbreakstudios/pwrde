...
- `settings_ui.rs` — Settings page content as an rcn element tree (Sessions/Keyboard/Terminal/Debug + search-results mode; Appearance stays canvas-painted in `renderer.rs`), same overlay pattern as `cleanup_ui.rs`. Keyboard capture (search typing, primary-command Input save/cancel, ⌘-chord recording) stays in `main.rs`'s `handle_settings_key`.
